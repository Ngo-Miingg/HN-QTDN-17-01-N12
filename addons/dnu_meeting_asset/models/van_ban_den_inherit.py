...
class VanBanDen(models.Model):
    ...
    def _update_source_on_approval(self, approved=True):
        """Ensure approval documents always sync back to asset/meeting flows."""
        result = super(VanBanDen, self)._update_source_on_approval(approved=approved)
        for rec in self:
            rec._sync_dnu_source_after_approval(approved=approved)
        return result

    def action_approve(self):
        result = super(VanBanDen, self).action_approve()
        for rec in self.filtered(lambda item: item.approval_state == 'approved'):
            rec._sync_dnu_source_after_approval(approved=True)
        return result

    def action_reject(self):
        result = super(VanBanDen, self).action_reject()
        for rec in self.filtered(lambda item: item.approval_state == 'rejected'):
            rec._sync_dnu_source_after_approval(approved=False)
        return result

    def _sync_dnu_source_after_approval(self, approved=True):
        self.ensure_one()
        if not self.source_res_id:
            return

        if self.request_type == 'booking_approval':
            source_record = self.env['dnu.meeting.booking'].sudo().browse(self.source_res_id).exists()
            if not source_record:
                return
            if approved and source_record.state == 'submitted':
                source_record.with_context(from_van_ban_approval=True).action_confirm()
                source_record.message_post(body='Đặt phòng đã được duyệt từ văn bản: %s' % self.ten_van_ban)
            elif not approved and source_record.state != 'cancelled':
                source_record.write({
                    'cancellation_reason': self.approval_note or 'Bị từ chối bởi Ban Giám đốc',
                })
                source_record.action_cancel()
                source_record.message_post(body='Đặt phòng đã bị từ chối từ văn bản: %s' % self.ten_van_ban)

        elif self.request_type == 'lending_approval':
            source_record = self.env['dnu.asset.lending'].sudo().browse(self.source_res_id).exists()
            if not source_record:
                return
            if approved and source_record.state in ['requested', 'pending_approval']:
                source_record.with_context(from_van_ban_approval=True).action_approve_lending()
                source_record.message_post(body='Mượn thiết bị đã được duyệt từ văn bản: %s' % self.ten_van_ban)
            elif not approved and source_record.state != 'cancelled':
                source_record.write({
                    'state': 'cancelled',
                    'approval_status': 'rejected',
                    'approval_note': self.approval_note or 'Bị từ chối bởi Ban Giám đốc',
                })
                source_record.message_post(body='Mượn thiết bị đã bị từ chối từ văn bản: %s' % self.ten_van_ban)
