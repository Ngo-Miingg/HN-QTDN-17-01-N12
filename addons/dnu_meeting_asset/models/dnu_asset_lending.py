...
class AssetLending(models.Model):
    ...
    @api.depends(
        'asset_id',
        'asset_id.assignment_ids',
        'asset_id.assignment_ids.state',
        'asset_id.assignment_ids.date_to',
        'asset_id.assignment_ids.date_from',
        'asset_id.assignment_ids.employee_id',
        'asset_id.assignment_ids.nhan_vien_id',
    )
    ...
    def action_lend(self):
        ...
        for lending in self:
            ...
            asset_update_vals = {'state': 'assigned'}
            if lending.asset_id.state == 'available':
                asset_update_vals.update({
                    'assigned_to': lending.borrower_id.id,
                    'assigned_nhan_vien_id': lending.nhan_vien_muon_id.id if lending.nhan_vien_muon_id else False,
                })
            lending.asset_id.write(asset_update_vals)
    ...
    def action_return(self):
        ...
        for lending in self:
            ...
            if not other_lending:
                asset_return_vals = {}
                if lending.asset_id.assigned_nhan_vien_id or lending.asset_id.assigned_to:
                    asset_return_vals['state'] = 'assigned'
                else:
                    asset_return_vals.update({
                        'state': 'available',
                        'assigned_to': False,
                        'assigned_nhan_vien_id': False,
                    })
                lending.asset_id.write(asset_return_vals)
    ...
    def _cron_check_overdue(self):
        ...
        for lending in overdue_lendings:
            ...
            if template:
                template.send_mail(lending.id, force_send=True, email_values={
