...
class HrEmployeeExtend(models.Model):
    ...
    def _check_unique_nhan_vien_link(self):
        for employee in self.filtered('nhan_vien_id'):
            duplicate = self.search([
                ('id', '!=', employee.id),
                ('nhan_vien_id', '=', employee.nhan_vien_id.id),
            ], limit=1)
            if duplicate:
                raise ValidationError(_(
                    'Nhân viên "%s" đã được liên kết với hồ sơ HR "%s".'
                ) % (employee.nhan_vien_id.ho_va_ten, duplicate.name))

    @api.depends(
        'nhan_vien_id.lich_su_cong_tac_ids',
        'nhan_vien_id.lich_su_cong_tac_ids.loai_chuc_vu',
        'nhan_vien_id.lich_su_cong_tac_ids.don_vi_id',
        'nhan_vien_id.lich_su_cong_tac_ids.chuc_vu_id',
    )
...
class NhanVienExtend(models.Model):
    ...
    @api.constrains('hr_employee_id')
    def _check_unique_hr_employee_link(self):
        for nv in self.filtered('hr_employee_id'):
            duplicate = self.search([
                ('id', '!=', nv.id),
                ('hr_employee_id', '=', nv.hr_employee_id.id),
            ], limit=1)
            if duplicate:
                raise ValidationError(_(
                    'Hồ sơ HR "%s" đã được liên kết với nhân viên "%s".'
                ) % (nv.hr_employee_id.name, duplicate.ho_va_ten))
